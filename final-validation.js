...
// The real Requirements/Documents columns are plain `text`, not an array or
// jsonb type — so the assembled record stores a JSON-serialized string, not
// a raw JS array. This checks that the string, when parsed, actually
// represents an array of strings — the same underlying invariant as
// isStringArray above, just applied to the serialized-text reality.
function isSerializedStringArray(value) {
...
function checkSchemaConformance(assembledRecord) {
  ...
  ['Requirements', 'Documents'].forEach(field => {
    ...
    if (value !== null && value !== undefined && !isSerializedStringArray(value)) {
      rejectReasons.push('Field "' + field + '" must be a JSON-serialized array of strings (the real column is plain text) when present (got ' + JSON.stringify(value) + ')');
    }
  });
