...
const { factsMatch, deadlinesMatch, isSerializedStringArray } = require('./final-validation');
...
function sanityCheckRecord(assembledRecord, sourceFacts) {
  ...
  ['Requirements', 'Documents'].forEach(field => {
    ...
    if (value !== null && value !== undefined && !isSerializedStringArray(value)) {
      reasons.push('Field "' + field + '" must be a JSON-serialized array of strings (the real column is plain text) when present at insertion time');
    }
  });
