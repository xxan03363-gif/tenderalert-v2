...
run('4. AI fields correctly mapped', { sourceFacts: baselineSourceFacts(), aiResult: baselineAiResult() }, true, r =>
  ...
  r.record.Requirements === JSON.stringify(['Requirement A', 'Requirement B']) &&
  r.record.Documents === JSON.stringify(['Document A'])
);
...
// 5b. Deadline with a full ISO datetime + timezone is normalized to plain YYYY-MM-DD,
// matching the real Tenders.deadline column type (date, no time component).
run('5b. Full ISO datetime deadline normalized to date-only', {
  sourceFacts: { ...baselineSourceFacts(), deadline: '2027-09-28T17:00:00+03:00' },
  aiResult: baselineAiResult()
}, true, r => r.record.deadline === '2027-09-28');

// 5c. Unparseable deadline is refused outright rather than passed through as garbage.
run('5c. Unparseable deadline -> refused', {
  sourceFacts: { ...baselineSourceFacts(), deadline: 'next Tuesday' },
  aiResult: baselineAiResult()
}, false);
