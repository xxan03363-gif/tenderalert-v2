...
function baseline() {
  ...
  return {
    ...
    assembledRecord: {
      ...
      Requirements: JSON.stringify(['Requirement A', 'Requirement B']),
...
run('10. Invalid requirements type (string instead of array)', t, 'REJECTED', r => r.reasons.join(' | ').includes('"Requirements" must be a JSON-serialized array of strings'));

t = clone(baseline());
t.assembledRecord.Documents = 12345;
run('11. Invalid documents type (number instead of array)', t, 'REJECTED', r => r.reasons.join(' | ').includes('"Documents" must be a JSON-serialized array of strings'));
